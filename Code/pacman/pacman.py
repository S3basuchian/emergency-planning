...
class GameState:
    ...
    def getLegalActions(self, agentIndex=0, filter=None, train=False,
                        supervise=False):
        """
        ...
        if agentIndex == 0:  # Pacman is moving
            legal = PacmanRules.getLegalActions(self, filter=filter,
                                                train=train,
                                                supervise=supervise)
            return legal
        else:
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        if self.isWin() or self.isLose(): raise Exception(
    ...
    def generatePacmanSuccessor(self, action):  #CHECK HERE
        """
...
class PacmanRules:
    ...
    def getLegalActions(state, filter=None, train=False, supervise=False):
        ...
        """
        actions = Actions.getPossibleActions(
            state.getPacmanState().configuration, state.data.layout.walls)
        if filter is not None and not train and supervise:
            legal = filter.process_message(
                filter.send_request(filter.build_query(state.data, actions,
                                                       'FILTER')))
        else:
            legal = actions
        return legal
...
class GhostRules:
    ...
    def collide(state, ghostState, agentIndex):
        if ghostState.isScared():
            ...
            #        writer.writerow(a)
            state.data.countEaten[agentIndex] += 1
            if agentIndex == 1:
...
def readCommand(argv):
    ...
    parser.add_option('-m', '--mode', dest='mode', type='int',
                      help=default(
                          '0 - create instances, 1 run rl, 2 run aspfr (vegan), 3 run aspfr(vegetarian)'),
                      metavar='MODE', default=1)
    parser.add_option('--horizon', dest='horizon', type='int',
                      help=default(
                          'horizon of ASP FR'),
                      metavar='HORIZON', default=1)
    ...
    parser.add_option('-z', '--zoom', type='float', dest='zoom',
                      ...
                      default=1.0)
    parser.add_option('-f', '--fixRandomSeed', type='int',
                      help=default('fixes the random seed'),
                      metavar='SEED', default=42)
    ...
    parser.add_option('--timeout', dest='timeout', type='int',
                      ...
                      default=30)
    parser.add_option('--norm', dest='norm', help=default(
        'Specify norm set: broken, busy, hungry, vegan, vegetarian'),
                      default=None)
    parser.add_option('--reason', dest='reason',
                      help=default('Specify reasoner type: DDPL'),
                      default=None)

    parser.add_option('--rec', dest='rec', help=default(
        'Would you like to save a record of tests run? Input file name.'),
                      default=None)
    parser.add_option('--supervise', action='store_true', dest='supervise',
                      help='Use normative supervisor?', default=False)
    parser.add_option('--learn', action='store_true', dest='learn1',
                      help='Learn with norms - only choose with MORL agent',
                      default=False)
    parser.add_option('--sublearn', action='store_true', dest='learn2',
                      help='Learn with sub ideal reward function; only select for SubIdealAgent',
                      default=False)
    ...
    parser.add_option('--port', type='int', dest='port',
                      help=default('Port number.'), default=6666)
    ...
    if 'mode' not in agentOpts: agentOpts[
        'mode'] = options.mode
    if 'name' not in agentOpts: agentOpts[
        'name'] = options.layout

    weights = None
    if options.mode != 0:
        name = options.layout.split('/')[-1].split('.')[0]
        weights = pickle.load(open('layouts/learning/' + agentOpts['extractor'] + '/' + name +'.pkl', 'rb'))

    pacman = pacmanType(weights, args['layout'].numGhosts, options.horizon + 1, options.radius, **agentOpts)  # Instantiate Pacman with agentArgs
    args['pacman'] = pacman
...
def loadAgent(pacman, nographics):
    ...
    for moduleDir in pythonPathDirs:
        ...
        moduleNames = [f for f in os.listdir(moduleDir) if
                       f.endswith('gents.py')]
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0,
             ...
             norm=None, reason=None, rec=None, supervise=False, learn1=False,
             learn2=False, partial=False, port=6666):
    import __main__
    ...
    for i in range(numGames):
        ...
        game = rules.newGame(layout, pacman, ghosts, gameDisplay, beQuiet,
                             catchExceptions, filt, train, sup, learn1, learn2)
        game.run()
    ...
    if (numGames - numTraining) > 0:
        ...
        if rec is not None:
            with open(rec + '.csv', 'w') as file:
                ...
                header = ['score', 'blue ghosts eaten', 'orange ghosts eaten',
                          'violations', 'win/lose']
                writer.writerow(header)
                for game in games:
                    row = [game.state.getScore(),
                           game.state.getGhostsEaten()[0],
                           game.state.getGhostsEaten()[1],
                           game.state.getViolations(), game.state.isWin()]
                    writer.writerow(row)
        ...
        if len(games) > 1:
            ...
            eatenBlue = [game.state.data.countEaten[1] for game in games]
            eatenRest = []
            for game in games:
                eaten = 0
                for agent in range(2, len(game.state.data.countEaten)):
                    eaten += game.state.data.countEaten[agent]
                eatenRest.append(eaten)
            interventions = [game.state.data.interventions for game in games]
            text += 'Average Score:' + str(sum(scores) / float(len(scores))) + '\n'
            text += 'Maximum Score:' + str(max(scores)) + '\n'
            text += 'Avg interventions:' + str(sum(interventions) / float(len(interventions))) + '\n'
            text += 'Avg Blue eaten:' + str(sum(eatenBlue) / float(len(eatenBlue))) + '\n'
            text += 'Avg Other eaten:' + str(sum(eatenRest) / float(len(eatenRest))) + '\n'
            text += 'Win Rate:      %d/%d (%.2f)' % (wins.count(True), len(wins), winRate) + '\n'
            ...
            for agent in range(2, len(game.state.data.countEaten)):
                eaten += game.state.data.countEaten[agent]
            text += 'Win: ' + str(int(game.state.isWin())) + '\n'
            text += 'Score: ' + str(game.state.getScore()) + '\n'
            text += 'Interventions: ' + str(game.state.data.interventions) + '\n'
            text += 'Blue Eaten: ' + str(game.state.data.countEaten[1]) + '\n'
            text += 'Other Eaten: ' + str(eaten) + '\n'
            if len(game.state.data.times[1:]) > 0:
                text += 'Average time: ' + str(sum(game.state.data.times[1:])/len(game.state.data.times[1:])) + '\n'
                text += 'Max time: ' + str(max(game.state.data.times[1:])) + '\n'
                text += 'Min time: ' + str(min(game.state.data.times[1:])) + '\n'
            else:
                text += 'Average time: ' + str(0) + '\n'
                text += 'Max time: ' + str(0) + '\n'
                text += 'Min time: ' + str(0) + '\n'
            text += 'Startup time: ' + str(game.state.data.times[0]) + '\n'
            text += 'Steps: ' + str(len(game.state.data.times)) + '\n'
